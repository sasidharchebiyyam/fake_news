...
# ----------------------- Real-time Fact Checking Function -----------------------
...
def check_real_time_facts(text):
    ...
    public_interest_keywords = ["prime minister of india", "capital of india", "capital of andhra pradesh",
                               ...
                               "stock market today", "weather in vizianagaram"] # More keywords
        ...
        if keyword in text.lower():
            try:
                ...
                if found_relevant_info:
                    ...
                    # Basic attempt to identify contradictions (can be improved)
                    if "nandhitha" in text.lower() and "prime minister of india" in keyword:
                        results[f"Real-time check for '{keyword}'"] += "*Potentially Incorrect:* Current Prime Minister is likely Narendra Modi based on common knowledge."
                    ...
                    results[f"Real-time check for '{keyword}'"] = "Could not find relevant information online for this query."

            except Exception as e:
                results[f"Real-time check for '{keyword}'"] = f"Error during web search: {e}"

    if not results:
        results["Real-time Analysis"] = "No specific public interest keywords detected for real-time fact-checking."
...
if st.button("Analyze"):
    if not user_input.strip():
        ...
        if real_time_check_results:
            ...
            st.info("No specific public interest claims detected for real-time fact-checking.")
        ...
        if st.checkbox("Save Analysis"):
            output_df = pd.DataFrame({
                ...
                "Confidence": [conf],
                "Real-time Fact Check": [real_time_check_results if real_time_check_results else "No real-time checks performed"]
            })
...
st.sidebar.info(
    "This tool attempts to detect fake news and verify public interest claims using a basic model and real-time web search.\n\n"
    "*Important Notes:*\n"
    "- Fact-checking accuracy heavily relies on the quality of web search results.\n"
    "- The fake news detection model is basic; advanced models offer better performance.\n"
    "- This tool should be used as a guide and not a definitive source of truth. Always verify information from multiple reliable sources."
)